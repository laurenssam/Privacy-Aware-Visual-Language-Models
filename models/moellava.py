...
class MoeLLava:
    def __init__(self, temperature=0, max_new_tokens=512):
        ...
        disable_torch_init()
        self.model_path = "LanguageBind/MoE-LLaVA-Phi2-2.7B-4e"
        self.device = "cuda:0"
        load_4bit, load_8bit = False, False
        model_name = get_model_name_from_path(self.model_path)
        self.tokenizer, self.model, processor, context_len = load_pretrained_model(
            self.model_path, None, model_name, load_8bit, load_4bit, device=self.device
        )
    ...
    def predict(self, imgs, prompt, in_context_answers=None):
        ...
        image_tensor = self.image_processor.preprocess([imgs[0]], return_tensors="pt")[
            "pixel_values"
