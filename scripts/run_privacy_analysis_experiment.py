...
from helpers import (
    ...
    _get_class,
    file_to_list,
    in_split_file, matthews_corrcoef_from_cm,
)
...
class Experiment:
    def __init__(
        ...
        self.output_folder = create_experiment_folder(
            output_folder / Path(dataset_name) / Path(model_name)
        )
    ...
    def run(self):
        """Run the experiment."""
        print(
            f"Running experiment with model '{self.model_name}' on dataset '{self.dataset_name}'."
        )
            ...
            for idy, prediction in enumerate(predictions):
                if self.translate and prediction:
                    ...
                    prediction = translator.translate(
                        prediction, src=self.translate, dest="en"
                    ).text
    ...
    def save_results(self, prediction, img_path, label):
        ...
        result_path = (
            ...
            / f"{img_path.stem}_{0 if label == 'public' else 1}.txt"
        )
    ...
    def evaluate(self):
        ...
        pretty_print = format_evaluation_output(
            metrics, "All Classes", self.rejections, self.confusion_matrix
        )
        ...
        save_pickle(
            ...
            self.output_folder / "metrics.pickle",
        )
        print(pretty_print)
    ...
    def evaluate_per_class(self):
        ...
        scores_per_class = {
            class_name: deepcopy(score_dict) for class_name in PRIVATE_CLASSES
        }
        total_negatives = {"tn": 0, "fp": 0}
        for split_file in self.split_folder.glob("*.txt"):
            split_integers = file_to_list(split_file)
            per_split_class_confusion_matrix = {
                class_name: {"tp": 0, "fn": 0} for class_name in PRIVATE_CLASSES
            }
            ...
            for idx, prediction_path in enumerate(self.predictions_folder.iterdir()):
                ...
                if label == 0 and not in_split_file(split_integers, prediction_path):
                    ...
                    total_negatives[confusion_label] += 1
                    continue
                else:
            ...
            for (
                class_name,
                confusion_matrix,
            ) in per_split_class_confusion_matrix.items():
                metrics_per_class = calculate_metrics_from_confusion_matrix(
                    confusion_matrix["tp"],
                    per_split_negatives["fp"],
                    confusion_matrix["fn"],
                    per_split_negatives["tn"],
                )
        ...
        for class_name, scores_dict in scores_per_class.items():
            ...
            tp = per_split_class_confusion_matrix[class_name]['tp']
            fp = total_negatives['fp']
            fn = per_split_class_confusion_matrix[class_name]['fn']
            tn = total_negatives['tn']
            scores_per_class[class_name]['mcc'] = matthews_corrcoef_from_cm(tp, tn, fp, fn)
            print(f"{class_name}: {scores_per_class[class_name]['mcc']}")
            pretty_print = format_evaluation_output(
                scores_per_class[class_name], class_name
            )
            write_to_file(
                self.output_folder / f"metrics_{class_name}.txt", pretty_print
            )
        save_pickle(
            {"metrics": scores_per_class},
...
def parse_arguments():
    ...
    parser.add_argument(
        "--prompt",
        type=Path,
        default="prompts/variation_1.txt",
    ...
    parser.add_argument(
        "--translate", type=str, default=None, help="Language to translate to"
    )
    parser.add_argument(
        "--tl_model", type=str, default=None, help="finetuned tinyllava model"
    )
