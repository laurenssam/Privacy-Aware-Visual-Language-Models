...
PRIVATE_CLASSES = ["Bank Statement", "Bill or Receipt", "Business Card", "Condom Box", "Debit Card", "Doctors Prescription", "Letter with Address", "Local News Paper", "Medical Record", "Mortage, Investment or Retirement Report", "Pill Bottle", "Pregnancy Test", "Tattoo", "Transcripts"]
...
class BivPriv(Dataset):
    def __init__(self, root_dir, transform=transforms.Resize(240)):
        ...
        self.private_images = [file for file in self.private_img_dir.iterdir() if file.suffix.lower() in image_extensions]
        self.public_images = [file for file in self.public_img_dir.iterdir() if file.suffix.lower() in image_extensions]
        random.shuffle(self.public_images)
        self.images = self.private_images + self.public_images[:len(self.private_images)]
