...
def balanced_accuracy(tp, fp, fn, tn):
    """
    Compute balanced accuracy: the average of sensitivity (true positive rate)
    ...
    Raises:
    - ValueError if there are no positive or no negative samples.
    """
    if tp + fn == 0:
        raise ValueError("No positive samples: tp + fn must be > 0")
    if tn + fp == 0:
        raise ValueError("No negative samples: tn + fp must be > 0")
...
if __name__ == "__main__":
    example_score = balanced_accuracy(tp=50, fp=10, fn=5, tn=100)
    print(f"Balanced Accuracy (example): {example_score:.4f}")


def matthews_corrcoef_from_cm(tp: int, tn: int, fp: int, fn: int) -> float:
    """
    Compute Matthews Correlation Coefficient from confusion-matrix entries.
    ...
    Returns:
        MCC in range [-1, 1]. Returns 0 if denominator is zero.
    """
    num = tp*tn - fp*fn
